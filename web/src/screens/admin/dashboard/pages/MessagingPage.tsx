...
export function MessagingPage() {
  ...
  return (
    <div className="space-y-6 max-w-[1440px] mx-auto pb-12">
      {/* Header Card */}
      <div className="bg-white p-6 rounded-[20px] border border-admin-line shadow-sm">
        <h2 className="text-[20px] font-bold text-admin-ink mb-1">Content / Messaging Management</h2>
      ...
      {!isLoading && !error && settings.length === 0 && (
        <div className="bg-white rounded-[20px] border border-admin-line shadow-sm p-12 text-center">
          <MessageSquare className="w-8 h-8 text-admin-muted mx-auto mb-3" />
          <h3 className="text-[15px] font-bold text-admin-ink">No templates configured</h3>
        </div>
      )}
      ...
      {!isLoading && !error && settings.map(s => {
        ...
        return (
          <div key={s.key} className="bg-white rounded-[20px] border border-admin-line shadow-sm overflow-hidden flex flex-col">
            {/* Card Header */}
            <div className="p-5 border-b border-admin-line bg-white flex items-center justify-between">
              <div>
                <h3 className="text-[15px] font-bold text-admin-ink flex items-center gap-2">
                  {s.label}
              ...
              <div className="flex gap-2 shrink-0">
                {channels.map(ch => (
                  <span key={ch} className="px-2.5 py-1 rounded-md bg-admin-surface border border-admin-line text-[11px] font-bold text-admin-muted uppercase tracking-wider">
                    {ch}
                  </span>
                ))}
              </div>
            ...
            <div className="p-5 flex flex-col xl:flex-row gap-6 bg-[#FAFAFA]">
              ...
              <div className="flex-1 flex flex-col">
                {variables.length > 0 && (
                  <div className="mb-3">
                    <span className="text-[12px] font-semibold text-admin-muted uppercase tracking-wider block mb-2">Available Variables</span>
                ...
                <textarea
                  ...
                  className="w-full h-40 md:h-52 p-4 rounded-xl border border-admin-line bg-white text-[14px] font-mono text-admin-ink shadow-sm outline-none focus:border-admin-brand focus:ring-1 focus:ring-admin-brand resize-none transition"
              ...
              {/* Preview Side */}
              <div className="flex-1 flex flex-col">
                <span className="text-[12px] font-semibold text-admin-muted uppercase tracking-wider block mb-2">Live Preview</span>
                <div className="flex-1 bg-white border border-admin-line rounded-xl p-5 shadow-sm relative overflow-hidden">
            ...
            <div className="p-5 border-t border-admin-line bg-white">
              ...
              <div className="flex justify-end gap-3">
                <button
                  ...
                  className="px-4 py-2 rounded-[8px] text-[13px] font-semibold text-admin-muted hover:bg-admin-surface hover:text-admin-ink disabled:opacity-50 disabled:cursor-not-allowed transition flex items-center gap-2"
                >
